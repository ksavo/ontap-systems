...
+
----
 cluster::> vserver services name-service dns shown
                                                               Name
...
.	Capture the information in the output of the command in Step 3 to use in the section <<Map cluster and node-management ports from node1 to node3>>.
+
...
|Did not record the NVRAM System ID number in the section <<Preparing the nodes for the upgrade>>
|Complete Step 11 and Step 12 and then go on to the next section, <<Retire node1>>.
|===
...
.	Record the NVRAM System ID number to use in the section <<Install and boot node3>>.
