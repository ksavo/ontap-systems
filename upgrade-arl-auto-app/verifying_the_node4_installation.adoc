...
If the physical ports on node2 do not map directly to the physical ports on node4, the subsequent section link:setting_the_fc_or_uta_uta2_configuration_on_node4.html[Set the FC or UTA/UTA2 configuration on node4] can be used to repair the network.
...
. If you are working on a MetroCluster system,  verify that the replaced controller is configured correctly for the MetroCluster configuration; the MetroCluster configuration should be in healthy state. Refer to link:_verifying_the_health_of_the_metrocluster_configuration.html[Verify the health of the MetroCluster configuration].
+
...
For more information on creating and re-creating VLANs, interface groups, and broadcast domains, see the _ONTAP 9 Network Management Guide_.
...
....
+
A warning message like the following is expected for ports with a reachability status that might be different from the reachability status of the broadcast domain where it is currently located.
+
....
