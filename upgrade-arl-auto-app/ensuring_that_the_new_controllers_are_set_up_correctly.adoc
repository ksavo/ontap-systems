...
`system license show`
+
You might want to compare the output with the output that you captured in the link:preparing_the_nodes_for_upgrade.html[Preparing the nodes for upgrade section].
...
If Storage Encryption is enabled on node3 and node4, complete the section link:setting_up_storage_encryption_on_the_new_controller_module.html[Setting up Storage Encryption on the new controller module]. Otherwise, complete the section link:decommissioning_the_old_system.html[Decommissioning the old system].
