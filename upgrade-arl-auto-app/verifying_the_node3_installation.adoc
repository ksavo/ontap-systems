...
. [[step1]]List all the physical ports that are on upgraded node1 (referred to as node3) by using the following command:
+
...
. [[step2]]List the broadcast-domains on the cluster by using the following command:
+
`broadcast-domain show`

. [[step3]]List network port reachability of all ports on node3 by using the following command:
+
...
. [[step10]]Verify port reachability by using the following command:
+
`network port reachability show`
+
