...
image::../media/pcm_replace_livermore_2_replacing_the_controller_hardware.gif[]
...
IMPORTANT: During the node reboot on systems with the Trusted Platform Module (TPM) installed and the onboard key manager configured, you must contact Customer Support if you see the following messages displayed: `...[....:crypto.ssal.failed:ALERT] : SSAL Unseal operation failed:  SSAL Unseal operation failed.
