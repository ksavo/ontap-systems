...
. If the node is at the LOADER prompt, boot the node. Respond `y` if you see a prompt warning of a system ID mismatch and then asking to override the system ID, and then enter `bye` when the override completes.
