...
. Verify that port e0b is up: `network port show -ipspace Cluster`
+
...
. Verify that the cluster network is healthy:``show isdp neighbors``
+
----
(cs1) # show isdp neighbors
Capability Codes: R - Router, T - Trans Bridge, B - Source Route Bridge,
...
See link:configure_log_collection.md#GUID-C8B9214A-2080-4813-B31C-6F2B85CBEA4D[Configuring the cluster switch log collection feature] for the steps required to enable cluster health switch log collection used for collecting switch-related log files.
