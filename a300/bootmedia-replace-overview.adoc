---
permalink: a300/bootmedia-replace-overview.html
sidebar: sidebar
keywords: aff a300, aff, a300, boot, media, store, primary, secondary, set, system, image, boot, network, configuration, perform, nondisruptive, disruptive, replacement, replace, replace the boot media
summary: The boot media stores a primary and secondary set of system \(boot image\) files that the system uses when it boots. Depending on your network configuration, you can perform either a nondisruptive or disruptive replacement.
---
= Overview of boot media replacement - AFF A300
:icons: font
:imagesdir: ../media/

[.lead]
include::../_include/bootmedia_replace_overview.adoc[]
