 ...
 ** If the command output lists the drive details with Mode & Key ID information, NSE is configured in use, and you need to verify the NSE configuration.
 ** If no disks are shown, NSE is not configured.
 ** If NVE and NSE are not configured, no drives are protected with NSE keys and it's safe to shut down the impaired node.
...
+
If the command fails, contact http://mysupport.netapp.com/[mysupport.netapp.com].
...
NOTE: Enter the customer's onboard key management passphrase at the prompt. If the passphrase cannot be provided, contact http://mysupport.netapp.com/[mysupport.netapp.com].
 ...
 .. Verify that the `Key Manager` type shows `onboard`, and then manually back up the OKM information:  `security key-manager key-query`
 .. Go to advanced privilege mode and enter `y` when prompted to continue: `set -priv advanced`
