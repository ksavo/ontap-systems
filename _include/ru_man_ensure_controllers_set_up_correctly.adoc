...
. If any volumes are offline, compare them with the list of offline volumes that you captured in  link:prepare_nodes_for_upgrade.html#step19d[Prepare the nodes for upgrade, Step 19 (d)], and bring online any of the offline volumes, as required, by entering the following command, once for each volume:
+
...
. If NetApp Storage Encryption (NSE) was in use on the configuration and you set the `setenv bootarg.storageencryption.support` command to "true" with the `kmip.init.maxwait` variable "off" (in link:install_boot_node3.html#step16[Step 16]), you need to reset the variable:
...
`system license show`
+
You might want to compare the output with the output that you captured in link:prepare_nodes_for_upgrade.html#step30[Prepare the nodes for upgrade, Step 30].
