...
You should validate in the cluster mixing rules whether new nodes can become part of the cluster with the existing nodes when the original controller is replaced. For more information about cluster mixing rules, see the _Hardware Universe_ at hwu.netapp.com.
...
NOTE: If the new system has fewer slots than the original system, or if it has fewer or different ports, you might need to add an adapter to the new system. See the _Hardware Universe_ on the NetApp Support Site for details about specific platforms.
