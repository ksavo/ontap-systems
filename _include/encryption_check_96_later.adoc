 ...
 ** If the `Key Manager` type displays `external` or `KMIP` and the `Restored` column displays `yes`, it's safe to shut down the impaired node.
...
. Display the key IDs of the authentication keys that are stored on the key management servers: `security key-manager query security key-manager key query -key-type NSE-AK`
