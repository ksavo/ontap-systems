...
+
For detailed information about the `storage aggregate relocation start`, `override-vetoes`, and `override-destination-checks` options, refer to link:other_referenes.html[References] to link to the _ONTAP 9 Commands: Manual Page Reference_.
...
. [[step1]]Enter the following command to get a list of aggregates:
+
...
. [[step2]]Compare the output of <<step1,Step 1>> with the output you captured for node1 in the section link:prepare_nodes_for_upgrade.html[Prepare the nodes for upgrade] and note any aggregates that were not correctly relocated.
