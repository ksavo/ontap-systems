...
[.lead]
Disk shelves with IOM12/IOM12B modules are supported in specific types of HA pair and single-controller configurations.
...
[.lead]
You can use mini-SAS HD SAS optical cables--multimode active optical cable (AOC) cables with mini-SAS HD-to-mini-SAS HD connectors and multimode (OM4) breakout cables with mini-SAS HD-to-LC connectors--to achieve long distance SAS connectivity for certain configurations that have disk shelves with IOM12/IOM12B modules.
