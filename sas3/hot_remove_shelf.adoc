...
+
Reassigning epsilon minimizes the risk of unforeseen errors impacting all nodes in a clustered ONTAP system. You can use the following steps to verify the node holding epsilon and reassign epsilon if needed:
+
. Set privilege level to advanced: `set -privilege advanced`
. Verify the node in the HA pair that is undergoing maintenance shows `false` in the output `Epsilon` column (does not hold epsilon).
