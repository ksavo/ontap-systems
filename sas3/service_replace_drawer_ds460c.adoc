...
[NOTE]
====
If when you replace a failed drive drawer, it results in the shelf having a combination of IOM12 modules and any number of DCM2 drive drawers, you must upgrade the IOM12 FW to version 0300 or later. The IOM12 modules require a version of FW supported with DCM2 drive drawers.
...
. Stop all host I/O activity by removing all aggregates from the disk drives (the disk drives must be spares).
+
