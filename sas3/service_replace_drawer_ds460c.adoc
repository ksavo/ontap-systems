...
* This procedure applies to shelves having DCM drive drawers and/or DCM2 drive drawers. (Drawers can also have two IOM12 or two IOM12B shelf modules.)
+
When a DCM or DCM2 drive drawer fails, you receive a DCM or DCM2 drive drawer to replace it.

NOTE: If when you replace a failed drive drawer, it results in the shelf having a combination of IOM12 modules and any number of DCM2 drive drawers, you must upgrade the IOM12 FW to version 0300 or later. The IOM12 FW upgrade can be done before or after replacing a drive drawer. This procedure has you upgrade the FW as part of the preparation for the drawer replacement procedure.
...
[.Lead]
Before you replace a drive drawer you must make sure to update IOM12 FW if needed, and then stop all host I/O activity and power off the shelf.
...
. Stop all host I/O activity on the shelf.
. Power off the shelf.
