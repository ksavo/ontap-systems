...
summary: Replacing a drive drawer in a shelf requires you to move all of the data from the aggregates residing on the disk drives, offline and delete the aggregates, and power off the shelf.
---
...
[.lead]
Replacing a drive drawer in a shelf requires you to move all of the data from the data aggregates residing on the disk drives, offline and delete the data aggregates, and power off the shelf. However, if the shelf contains a root aggregate, you must halt the entire storage system (HA pair).
...
[NOTE]
====
If when you replace a failed drive drawer, it results in the shelf having a combination of IOM12 modules and any number of DCM2 drive drawers, you must upgrade the IOM12 FW to version 0300 or later. The IOM12 modules require a version of FW that supports DCM2 drive drawers.
...
[.Lead]
Before you replace a drive drawer you must make sure to update IOM12 FW if needed, move all of the data from the data aggregates residing on the disk drives, offline and delete the data aggregates, and power off the shelf. However, if the shelf contains a root aggregate, you must halt the entire storage system (HA pair).
...
. If the shelf contains the root aggregate from either controller in the HA pair, complete the following substeps; otherwise, go to the next step.
+
...
+
NOTE: If you attempt this procedure with aggregates on the disk shelf, you could cause a system disruption with a multidisk panic.
...
. Verify that the disk drives have no aggregates (are spares):
 ...
 .. Check the output to verify that there are no aggregates on the disk drives.
+
...
* You have halted your HA pair or you have moved all of the data from the data aggregates residing on the disk drives, and offlined and deleted the data aggregates to allow the disk drives to become spares.
