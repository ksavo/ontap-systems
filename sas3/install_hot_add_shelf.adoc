...
If disk ownership automatic assignment is enabled, the output shows "`on`" (for each controller) in the "`Auto Assign`" column.
 ...
 .. If disk ownership automatic assignment is enabled, you need to disable it:``storage disk option modify -node node_name -autoassign off``
+
...
. Verify SAS connectivity for each hot-added disk shelf: `storage shelf show -shelf shelf_name -connectivity`
+
