...
+
----
command failed: This operation is not permitted on a Vserver that is configured
                as the destination of a MetroCluster Vserver relationship.
----
+
...
. [[step6]]Adjust the intercluster broadcast domains and migrate the intercluster LIFs, if necessary, using the same commands shown in <<step5,Step 5>>.

. [[step7]]Adjust any other broadcast domains and migrate the data LIFs, if necessary, using the same commands shown in <<step5,Step 5>>.
...
----
+
Go to link:other_references.html[Other references] to link to the _ONTAP 9 Network Management Guide_ or _ONTAP 9 Commands: Manual Page Reference_ for more information.
