...
. Verify that the expected volumes are present for each controller: `vol show -node _node-name_`
. If you disabled automatic takeover on reboot, enable it from the healthy controller: `storage failover modify -node _replacement-node-name_ -onreboot true`
