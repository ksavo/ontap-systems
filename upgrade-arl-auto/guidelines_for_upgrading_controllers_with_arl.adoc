...
NOTE: If you are upgrading a system that supports internal drives (for example, an FAS2700 or AFF A250) but does NOT have internal drives, use the procedure in _Using Aggregate Relocation to Manually Upgrade Controller Hardware_. Refer to link:other_references.html[Other references] to link to the _Using Aggregate Relocation to Manually Upgrade Controller Hardware_ guides.
