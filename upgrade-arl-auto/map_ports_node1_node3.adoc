...
Capture information about the ports on the new nodes from the Hardware Universe at hwu.netapp.com. You will use the information later in this section.
...
.. Boot node3. See the Installing and booting node3 on page 26 section to boot the node if you have not already done so.
...
. Adjust the broadcast domain membership of network ports used for intercluster LIFs using the same commands shown in Step 5.
