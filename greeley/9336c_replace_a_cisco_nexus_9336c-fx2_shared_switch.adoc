---
sidebar: sidebar
permalink: 9336c_replace_a_cisco_nexus_9336c-fx2_shared_switch.html
keywords:
summary:
---
...
Total entries displayed: 4Steps
....
...
.. On the new switch, log in as admin and shut down all the ports that will be connected to the node cluster interfaces (ports 1/1 to 1/34).
...
. Shut down the ISL ports 1/35 and 1/36 on the Nexus 9336C-FX2 switch sh1.
...
.. Remove all the cables from the Nexus 9336C-FX2 sh2 switch, and then connect them to the same ports on the Nexus C9336C-FX2 newsh2 switch.
...
** `system switch ethernet log setup-`
** `password system switch ethernet `
** `log enable-collection`
...
. Move the storage ports from the old switch sh2 to the new switch newsh2.
. Verify the storage attached to HA pair 1, shared switch newsh2is healthy.
...
. Verify that the shelves are correctly cabled: `storage shelf port show -fields remote- device,remote-port`.
