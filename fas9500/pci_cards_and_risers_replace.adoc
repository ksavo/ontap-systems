...
NOTE: If the new I/O module is not the same model as the failed module, you must reboot the BMC.

.Steps
. Reboot the BMC if the replacement module is not the same model as the old module
.. From the LOADER prompt, Go to advanced privilege mode: `set -priv advanced`
.. reboot the BMC: `sp reboot`
.. Exit advanced mode: `set -priv admin`  
. From the LOADER prompt, reboot the node: `bye`

NOTE: This reinitializes the PCIe cards and other components and reboot the node.
