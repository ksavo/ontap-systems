...
Technical support might recommend modifying the default settings on some of the tests to help identify the problem.
 ...
 .. Modify the selection state of a specific device or type of device on your storage system by entering the following command:
+
 `sldiag device modify [-dev _devtype_|mb|slot_slotnum_] [-name _device_] [-selection _enable|disable|default|only_]`
+
